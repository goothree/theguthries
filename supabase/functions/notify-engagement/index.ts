// notify-engagement/index.ts
// Called when someone reacts or comments on a post — notifies the post author.
...
// ── JWT helpers (same as notify-new-post) ─────────────────────
...
serve(async (req) => {
  const { postId, postAuthorId, engagementType, actorName } = await req.json();
  ...
  // Get all device tokens for the post author
  ...
  const message = engagementType === "comment"
    ? `${actorName} commented on your post`
    : `${actorName} reacted to your post`;

  const results = await Promise.all(
    tokens.map(({ token }) =>
      sendApns(token, "FamJam \uD83C\uDFE1", message, jwt, postId)
    )
  );

  const succeeded = results.filter(r => r.ok).length;
  const failed    = results.filter(r => !r.ok).map(r => `${r.status} ${r.reason}`);
  console.log(`notify-engagement: sent=${succeeded} failed=${failed.length}`, failed);
